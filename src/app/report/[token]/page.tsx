...
import { CheckCircle, Clock, X, BookOpen, Code2, Target, Briefcase, TrendingUp, TrendingDown, Award, AlertCircle, Lock, MessageSquare, Eye } from "lucide-react";
...
export default async function CandidateReportPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#f8fafc", fontFamily: "'Inter', -apple-system, sans-serif", display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ borderBottom: '1px solid var(--border)', background: 'linear-gradient(135deg, var(--brand-navy) 0%, #1e3a8a 100%)', flexShrink: 0 }}>
        <div style={{ maxWidth: '1440px', margin: '0 auto', padding: '1.25rem 2rem', display: 'flex', alignItems: 'center', gap: '1rem', width: '100%' }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.8rem', margin: 0 }}>{mappedCandidate.jobApplied} · {mappedCandidate.email}</p>
          </div>
      ...
      {/* Main Grid Area */}
      <div style={{ flex: 1, display: 'flex', gap: '1.5rem', padding: '1.5rem 2rem', backgroundColor: '#f8fafc', maxWidth: '1440px', margin: '0 auto', width: '100%' }}>
        
        {/* COLUMN 1: Scores & Skill Match (width: 32%) */}
        <div style={{ width: '32%', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ backgroundColor: '#fff', padding: '1.25rem', borderRadius: '12px', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', fontSize: '0.78rem', color: 'var(--gray-700)' }}>
                <span>📧 {mappedCandidate.email}</span>
                <span>📞 {mappedCandidate.phone || 'N/A'}</span>
                <span>💼 {mappedCandidate.jobApplied}</span>
                <span>🏷️ Stage: {mappedCandidate.stage || 'N/A'}</span>
              </div>
            </div>
          ...
          <div style={{ backgroundColor: '#fff', padding: '1.25rem', borderRadius: '12px', border: '1px solid var(--border)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0', overflowX: 'auto' }}>
              {[
                { label: 'Resume', status: mappedCandidate.resumeStatus },
                { label: 'Video', status: mappedCandidate.videoStatus },
                { label: 'Technical', status: mappedCandidate.techStatus },
                { label: 'Report', status: mappedCandidate.reportStatus },
              ].map((step, i, arr) => {
