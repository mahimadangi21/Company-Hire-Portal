...
DROP POLICY IF EXISTS "Allow all operations for authenticated users" ON public.questions_bank;
CREATE POLICY "Allow all operations for authenticated users" ON public.questions_bank FOR ALL USING (true);

DROP POLICY IF EXISTS "Allow all operations for authenticated users" ON public.interviews;
